...
const PROSPECTING_RECIPE_PREFIX = "/Lotus/Types/Recipes/DeimosRecipes/Prospecting/";

function safeString(value: unknown): string | null {
    return typeof value === "string" && value.trim().length > 0 ? value.trim() : null;
}

export function deriveDeimosProspectingBlueprintAcquisitionByCatalogId(): Record<string, AcquisitionDef> {
    ...
    for (const [catalogId, rec] of Object.entries(recordsById)) {
        ...
        if (!path?.startsWith(PROSPECTING_RECIPE_PREFIX)) continue;

        out[catalogId] = { sources: ["data:vendor/deimos/otak"] };
    }
